...
/*    protected   $_del_callback    = NULL;
    ...
    public function add(&$data, $onDuplicateKey=NULL)
    {
        ...
        */
        if ( $this->autoPrimaryKey == true
            ...
            $data[$this->primary_key] = self::genUUID();
        }
        ...
        */
        if ( $this->autoPrimaryKey == true ) {
            foreach ( $data as $key => $val ) {
                ...
                $val[$this->primary_key] = self::genUUID();
                $data[$key] = $val;
            }
        }
    ...
    */
    public function update(&$data, $where, $affected_rows=true)
    {
        ...
        $ret    = $this->db->update($this->table, $data, $_where, true, $affected_rows);
        ...
        foreach ( $sData as $fragment ) {
            $t = $fragment['table'];
            $d = $fragment['data'];
            $w = "{$fragment['primary_key']} {$idstr}";
            $r = $this->db->update($t, $d, $w, true, $affected_rows) || $r;
        }
